...
const VendorDashboard = () => {
  ...
  const [vendorData, setVendorData] = useState({
    id: null, vendorName: "", category: "", phoneNumber: "", email: "",
    address: "", website: "", logoUrl: "", products: [], joinDate: "2024-01-15"
  });

  const [stats, setStats] = useState({
    totalProducts: 0, totalViews: 1247, totalInquiries: 89,
    averageRating: 4.6, monthlyRevenue: 125000, activeBookings: 7
  });
  ...
  const [productFormData, setProductFormData] = useState({
    name: '', type: '', description: '', price: '', imageUrl: '', vendorId: vendorId
  });

  const [profileFormData, setProfileFormData] = useState({
    vendorName: '', category: '', phoneNumber: '', email: '', address: '', website: '', logoUrl: ''
  });
  ...
  const fetchVendorProfile = async () => {
    ...
    if (result.success) {
      ...
      setProfileFormData({
        vendorName: result.data.vendorName || '', category: result.data.category || '',
        phoneNumber: result.data.phoneNumber || '', email: result.data.email || '',
        address: result.data.address || '', website: result.data.website || '',
  ...
  const handleAddProduct = async (e) => {
    ...
    if (result.success) {
      ...
      setProductFormData({ name: '', type: '', description: '', price: '', imageUrl: '', vendorId: vendorId });
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="text-center max-w-lg bg-white rounded-xl p-6 shadow-lg">
          ...
          <p className="text-gray-600 mb-4 whitespace-pre-line text-sm">{error}</p>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="bg-red-50 border border-red-200 rounded-lg p-4">
          ...
          <div className="text-sm text-red-700 space-y-1">
            ...
            <ul className="list-disc list-inside ml-4 text-xs">
              ...
              <li>PUT /api/vendor/updateVendorById/{`{id}`} (for updating vendor)</li>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-gray-600">
                  {vendorData.category || "Select category"} • Member since {new Date(vendorData.joinDate).toLocaleDateString()}
                </p>
            ...
            <div className="space-y-3">
              {recentInquiries.map((inquiry) => (
                <div key={inquiry.id} className="p-3 border rounded-lg">
                  ...
                  <p className="text-xs text-gray-500 mb-2">{inquiry.message}</p>
                  <div className="flex items-center justify-between">
                    <span className="text-xs text-gray-400">
                      {new Date(inquiry.date).toLocaleDateString()}
                    </span>
                    <button className="text-[#a92b4e] text-xs hover:underline">Respond</button>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className="bg-white rounded-xl shadow-sm p-6 border">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {[
              { icon: Phone, label: "Phone", value: vendorData.phoneNumber || "Add phone", color: "blue" },
              { icon: Mail, label: "Email", value: vendorData.email || "Add email", color: "green" },
              { icon: Globe, label: "Website", value: vendorData.website || "Add website", color: "purple" },
              { icon: Building2, label: "Category", value: vendorData.category || "Select category", color: "orange" }
            ].map((item, index) => (
      ...
      {showEditProfileModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="flex items-center justify-between p-4 border-b bg-[#a92b4e] text-white rounded-t-xl">
              <h2 className="font-bold">{vendorData.id ? 'Edit Profile' : 'Create Profile'}</h2>
            ...
            <form onSubmit={handleUpdateProfile} className="p-4 space-y-4">
                ...
                <div className="relative inline-block">
                  <div className="w-20 h-20 rounded-full bg-gradient-to-r from-[#a92b4e] to-[#891737] flex items-center justify-center overflow-hidden">
                    {logoPreview || profileFormData.logoUrl ? (
                      <img src={logoPreview || profileFormData.logoUrl} alt="Logo" className="w-full h-full object-cover" />
                    ) : (
                      <User className="w-8 h-8 text-white" />
                    )}
                  </div>
              ...
              <div className="flex gap-3 pt-2">
                ...
                >
                  ...
                  {profileUpdateLoading ? 'Saving...' : (vendorData.id ? 'Update' : 'Create')}
                </button>
              </div>
            </form>
          </div>
