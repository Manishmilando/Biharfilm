...
const VendorProductForm = () => {
  ...
  const productTypes = {
    'Photography & Videography': ["Cameras", "Lighting", "Sound", "Grip", "Drones"],
    'Catering Services': ["Food Catering", "Beverage Services", "Equipment Rental", "Staff Services"],
    'Decorations & Lighting': ["Furniture", "Decor", "Lighting Equipment", "Temporary Structures"],
    'Music & Entertainment': ["Sound Systems", "Musical Instruments", "DJ Equipment", "Entertainment Services"],
    'Transportation': ["Vanity Vans", "Generators", "Cranes", "Trolleys"],
    'Venues & Locations': ["Sets", "Green Screen", "Outdoor Props", "Temporary Structures"],
    'Costumes & Makeup': ["Traditional", "Modern", "Fantasy", "Historical", "Uniforms"],
    'Equipment Rental': ["Cameras", "Lighting", "Sound", "Grip", "Drones"],
    'Post-Production': ["Editing Systems", "VFX Workstations", "Color Grading Tools"],
  ...
  // Update product keys when selectedTypes changes
  useEffect(() => {
  ...
  const handleTypeChange = (cat, type, checked) => {
    ...
    // If unchecking, clear the product data
    if (!checked) {
      setProductData(prev => {
        const updated = { ...prev };
        delete updated[key];
  ...
  const handleProductSubmit = async (e) => {
    ...
    try {
      ...
      if (validProducts.length === 0) {
        alert("Please add at least one complete product with all required fields.");
        return;
      }
      ...
      // Handle products - send as individual fields like in the attached file
        ...
        <form onSubmit={handleVendorSubmit} className="space-y-6">
          ...
          <div>
            ...
            {vendorErrors.vendorName && (
              <p className="text-red-500 text-sm mt-1">{vendorErrors.vendorName}</p>
            )}
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              {vendorErrors.phoneNumber && (
                <p className="text-red-500 text-sm mt-1">{vendorErrors.phoneNumber}</p>
              )}
            </div>
            <div>
              ...
              />
              {vendorErrors.email && (
                <p className="text-red-500 text-sm mt-1">{vendorErrors.email}</p>
              )}
            </div>
          </div>
          ...
          <div>
            ...
            {vendorErrors.address && (
              <p className="text-red-500 text-sm mt-1">{vendorErrors.address}</p>
            )}
          </div>
          ...
          <div>
            ...
            {vendorErrors.website && (
              <p className="text-red-500 text-sm mt-1">{vendorErrors.website}</p>
            )}
          </div>
              ...
              >
                {logoPreview ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-xs text-gray-400">Max size: 10MB, Format: JPG, PNG, GIF</p>
                  </div>
                )}
              </label>
            </div>
          ...
          <div>
            ...
            {vendorErrors.category && (
              <p className="text-red-500 text-sm mt-1">{vendorErrors.category}</p>
            )}
          </div>
      ...
      {step === 2 && (
        <form onSubmit={handleProductSubmit} className="space-y-6">
          <div className="flex items-center justify-between">
            <h2 className="text-2xl font-bold text-[#a92b4e]">Product Details</h2>
                ...
                {(productTypes[cat] || []).map((type) => (
                  <label key={type} className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={(e) => handleTypeChange(cat, type, e.target.checked)}
                      className="w-4 h-4 text-[#a92b4e] focus:ring-[#a92b4e]"
                    />
          ...
          {productKeys.length === 0 && (
            <div className="text-center py-8">
              <p className="text-gray-500">Select product types above to add their details</p>
            </div>
          )}
